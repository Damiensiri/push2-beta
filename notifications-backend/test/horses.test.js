...
test('fondations chevaux sur D1 : migration, permissions, conservation, concurrence et budgets',async t=>{
 ...
 await t.test('sanitaire : historique, permissions, invalidation atomique, propriétaires, Cron et doublons',async()=>{
  ...
  assert.equal((await call(client+'/health',{method:'POST',token:'client1',body:input})).status,403);
  ...
  const created=await call(base,{method:'POST',body:input});assert.equal(created.status,201,JSON.stringify(created));const record=created.data.record;
  ...
  const history=await call(client+'/health',{token:'client1'});assert.equal(history.data.records.length,1);assert.equal(history.data.records[0].comment,input.comment);
  const changed=await call(base+'/'+record.id,{method:'PATCH',body:{...input,nextDueOn:shift(14),version:1}});assert.equal(changed.status,200);
  ...
  assert.equal((await call(base+'/'+next.id,{method:'DELETE',body:{version:1}})).status,200);
