...
window.PaddockHorses={
  ...
  async edit({reservation,api,optionsPath,path,onSaved}){
    ...
    dialog.innerHTML=`<form><h2>Modifier les chevaux</h2><p data-booking></p><div data-horses></div><p role="status" aria-live="polite"></p><div style="display:flex;gap:12px"><button type="submit" disabled>Enregistrer</button><button type="button" data-close>Fermer</button></div></form>`;
    dialog.querySelector('[data-booking]').textContent=`${reservation.date} · ${reservation.time} · ${{maison:'Maison',grande:'Grande voie',beudot:'Beudot'}[reservation.paddock]||reservation.paddock} · ${reservation.duration} min`;
    ...
    form.onsubmit=async event=>{event.preventDefault();submit.disabled=true;status.textContent='Enregistrement…';
      try{await api(path,{method:'PATCH',body:JSON.stringify({version:reservation.version,horseIds:this.selected(choices)})});dialog.close();await onSaved();}
